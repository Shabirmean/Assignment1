...
struct StateMsg_
{
  ...
  _Speed_type Speed;
   ...
   typedef std::vector<float, typename ContainerAllocator::template rebind<float>::other >  _LegCurrents_type;
  _LegCurrents_type LegCurrents;
...
struct MD5Sum< ::aquacore::StateMsg_<ContainerAllocator> >
{
  ...
  static const uint64_t static_value1 = 0x8f3da9637657db86ULL;
  static const uint64_t static_value2 = 0x21e960d32a985a35ULL;
};
...
struct Printer< ::aquacore::StateMsg_<ContainerAllocator> >
{
  template<typename Stream> static void stream(Stream& s, const std::string& indent, const ::aquacore::StateMsg_<ContainerAllocator>& v)
  {
    ...
    s << indent << "LegCurrents[]" << std::endl;
    for (size_t i = 0; i < v.LegCurrents.size(); ++i)
    {
      s << indent << "  LegCurrents[" << i << "]: ";
      Printer<float>::stream(s, indent + "  ", v.LegCurrents[i]);
    }
  }
};
